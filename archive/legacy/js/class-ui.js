...
(function () {
    ...
    function classTreeNode(classId, text, nodeClass, colorStyle) {
        ...
        const style = colorStyle ? ` style="${esc(colorStyle)}"` : '';
        return `<div class="class-tree-node ${nodeClass || ''}"${style}><span class="class-tree-icon"${classIconStyle(classId)}></span><span>${esc(text)}</span></div>`;
    }
    ...
    window.ClassUI = class ClassUI {
        ...
        _showStatusBuffTooltip(slot, e) {
            ...
            tip.innerHTML = `<div class="class-skill-tip-name">${esc(name)}</div>`
                + (summary ? `<div class="class-skill-tip-desc">${esc(summary)}</div>` : '')
                + (remain ? `<div class="class-skill-tip-meta">剩余 ${esc(remain)}</div>` : '');
        ...
        _showSkillTooltip(slotIndex, e) {
            ...
            const esc = window.escapeHtml || ((v) => String(v == null ? '' : v));
            tip.innerHTML = `<div class="class-skill-tip-name">${esc(sk.name)}</div>`
                + `<div class="class-skill-tip-desc">${esc(desc)}</div>`;
            tip.style.display = 'block';
            this._moveSkillTooltip(e);
        }
        ...
        _renderClassSelectCards() {
            ...
            BASE_CLASS_IDS.forEach(id => {
                ...
                card.innerHTML = `
                    ...
                    <h3>${esc(def.name)}</h3>
                    <p class="class-select-role">${esc(def.role || '')} · ${esc(def.description || '')}</p>
                    <div class="class-select-stats">
                        <span>HP ${esc(stats.hp)}</span><span>ATK ${esc(stats.attack)}</span>
                        <span>MATK ${esc(stats.magicAttack)}</span><span>SPD ${esc(stats.speed)}</span>
                    </div>
                    <button type="button" class="pe-btn pe-btn--primary" data-base-class="${esc(id)}">选择 ${esc(def.name)}</button>
                `;
        ...
        _renderTalentPanel(p) {
            ...
            (tree.branches || []).forEach(branch => {
                html += `<div class="talent-branch"><h4>${esc(branch.name)}</h4><div class="talent-nodes">`;
                (branch.nodes || []).forEach(node => {
                    const on = !!alloc[node.id];
                    const can = window.canAllocateTalent(p, node.id);
                    const locked = !on && !can.ok;
                    const title = esc(`${node.name}${node.ultimate ? '（终极）' : ''}`);
                    html += `<button type="button" class="talent-node${on ? ' active' : ''}${locked ? ' locked' : ''}" data-talent-id="${esc(node.id)}" title="${title}">${esc(node.tier)}${on ? '✓' : ''}</button>`;
                });
                html += '</div></div>';
            });
            wrap.innerHTML = html;
        ...
        _renderSkillPanel() {
            ...
            allProg.forEach((skillId, idx) => {
                ...
                const iconHtml = skillIconUrl
                    ? `<span class="skill-panel-icon" style="background-image:url(${esc(skillIconUrl)})"></span>`
                    : '<span class="skill-panel-icon"></span>';
                row.innerHTML = `
                    <div class="skill-panel-row-head">
                        ${iconHtml}
                        <div class="skill-panel-row-text">
                        <strong>${esc(displayDef.name)}</strong>
                        </div>
                    </div>
                    <p class="skill-panel-desc">${esc(desc)}</p>
                `;
