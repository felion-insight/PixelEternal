...
function isWeaponTypeRanged(weaponType) {
    if (!weaponType) return false;
    if (weaponType === 'ranged') return true;
    const cfg = typeof window !== 'undefined' && window.WEAPON_AFFINITY_CONFIG
        ? window.WEAPON_AFFINITY_CONFIG.weaponTypes
        : null;
    return !!(cfg && cfg[weaponType] && cfg[weaponType].category === 'ranged');
}

function isPlayerWeaponRanged(weapon, player) {
    if (weapon) {
        return isWeaponTypeRanged(weapon.weaponType) || RANGED_WEAPON_NAMES.includes(weapon.name);
    }
    if (player && player.classData && typeof window.getPlayerBaseClassId === 'function') {
        const base = window.getPlayerBaseClassId(player.classData);
        return base === 'mage' || base === 'archer';
    }
    return false;
}
...
function resolveProjectileSpriteIdForPlayerWeapon(weapon) {
    ...
    const bowDefault = 'proj_player_windfang';
    const magicDefault = 'proj_mob_void_star';
    if (!weapon) return bowDefault;
    if (weapon.name && map && map.weaponByName && map.weaponByName[weapon.name]) {
        return map.weaponByName[weapon.name];
    }
    const wt = weapon.weaponType;
    if (wt === 'staff' || wt === 'book' || wt === 'orb' || wt === 'rune') return magicDefault;
    return bowDefault;
}
...
class FloatingText {
    ...
    draw(ctx) {
        ...
        const isCritStyle = this.fontSize >= 24 || this.color === '#ffd700' || this.color === '#ff0000';
        const popScale = isCritStyle
            ? 1 + Math.sin(progress * Math.PI) * 0.22
        ...
        ctx.lineWidth = Math.max(2, this.fontSize / 4.5);
...
class DroppedItem {
    ...
    update(gameInstance, pickupTriggered = false) {
        ...
        const qualityLevels = { normal: 1, magic: 2, rare: 3, epic: 4, legendary: 5, mythic: 6,
            common: 1, fine: 2 };
        const q = this.item.quality || 'normal';
        const level = qualityLevels[q] || 1;
        const maxIntensity = level / 6;
    ...
    draw(ctx) {
        const q = this.item.quality || 'normal';
        const qualityColor = QUALITY_COLORS[q] || '#ffffff';
        const beamHeights = { normal: 0, magic: 10, rare: 22, epic: 34, legendary: 46, mythic: 58 };
        const beamH = beamHeights[q] || 0;

        if (beamH > 0) {
            const pulse = 0.65 + this.glowIntensity * 0.35;
            const grad = ctx.createLinearGradient(this.x, this.y, this.x, this.y - beamH * pulse);
            grad.addColorStop(0, qualityColor + 'cc');
            grad.addColorStop(0.5, qualityColor + '66');
            grad.addColorStop(1, qualityColor + '00');
            ctx.fillStyle = grad;
            ctx.fillRect(this.x - 4, this.y - beamH * pulse, 8, beamH * pulse);
        }
...
class Monster {
    constructor(x, y, type, gameInstance = null) {
        ...
        if (typeof window.initMonsterBreakGauge === 'function') {
            window.initMonsterBreakGauge(this);
        }
    }
    ...
    update(player) {
        ...
        if (typeof window.tickMonsterCombatStatuses === 'function') {
            window.tickMonsterCombatStatuses(this, this.gameInstance);
        }
    ...
    takeDamage(amount) {
        ...
        if (killed && this.gameInstance && this.gameInstance.player
            && typeof window.onBuildMonsterKilled === 'function') {
            window.onBuildMonsterKilled(this.gameInstance.player, this, this.gameInstance);
        }
        return killed;
    }
    ...
    draw(ctx, playerLevel = 1) {
        ...
        if (this.breakGauge && this.breakGauge.max > 0) {
            const bg = this.breakGauge;
            const breakY = this.y - this.size / 2 - 16;
            const breakRatio = bg.max > 0 ? Math.max(0, Math.min(1, bg.current / bg.max)) : 0;
            ctx.fillStyle = '#220033';
            ctx.fillRect(this.x - barWidth / 2, breakY, barWidth, 3);
            ctx.fillStyle = bg.broken ? '#ee66ff' : '#8844cc';
            ctx.fillRect(this.x - barWidth / 2, breakY, barWidth * breakRatio, 3);
        }
...
class TownScene {
        ...
        if (assetManager) {
            for (const building of Object.values(this.buildings)) {
                if (highlightType && building.type === highlightType) {
                    ...
                    ctx.save();
                    ctx.strokeStyle = `rgba(255, 220, 80, ${0.55 + pulse * 0.35})`;
                    ctx.lineWidth = 3;
                    ctx.beginPath();
                    ctx.arc(building.x, building.y, building.size / 2 + 10 + pulse * 6, 0, Math.PI * 2);
                    ctx.stroke();
                    ctx.font = 'bold 16px "Courier New", monospace';
                    ctx.fillStyle = '#ffd700';
                    ctx.fillText('!', building.x, building.y - building.size / 2 - 12);
                    ctx.restore();
                }
                const imageName = assetManager.getBuildingImageName(building.type);
                if (imageName) {
    ...
    _drawBuildingFallback(ctx, building) {
        ...
        const colors = {
            ...
            jeweler: { fill: '#604020', stroke: '#ffcc00' },
            chronicle_stone: { fill: '#404050', stroke: '#aaaacc' },
            awakening_gate: { fill: '#204060', stroke: '#00ffff' },
            material_realm: { fill: '#304030', stroke: '#88ff88' }
        };
...
class TrialScene {
    ...
    spawnBoss(bossId, meta) {
        this.reset();
        this.title = (meta && meta.title) || '试炼';
        this.hint = (meta && meta.hint) || '';
        const bx = this.width / 2;
        const by = this.height / 2 - 40;
        this.boss = new Boss(bx, by, bossId, this.gameInstance);
        this.boss._isTrialBoss = true;
        return this.boss;
    }

    getMonsters() {
        return this.boss && this.boss.hp > 0 ? [this.boss] : [];
    }
    ...
    draw(ctx) {
        ...
        ctx.font = 'bold 16px "Courier New", monospace';
        ctx.textAlign = 'center';
        ctx.fillText(this.title || '试炼', this.width / 2, 24);
        ctx.fillStyle = '#bbaacc';
        ctx.font = '12px "Courier New", monospace';
        ctx.fillText(this.hint || '', this.width / 2, 44);
...
function boostMonsterTowardLevel(monster, targetLevel) {
    ...
    const rewardMult = Math.pow(1.048, delta);
...
/** 暴击伤害倍率：基础 150% + 暴伤属性% */
function applyCritDamageMultiplier(baseDamage, critDamageStat, extraCritMult = 1) {
    const stat = Math.max(0, (critDamageStat || 0) * (extraCritMult || 1));
    return Math.floor(baseDamage * (1.5 + stat / 100));
}
        ...
        if (this.type === ROOM_TYPES.BATTLE) {
            ...
            for (let i = 0; i < remainingCount; i++) {
                ...
                if (spawnPlayerLevel > 20 && Math.random() < 0.3) {
                    ...
                    // 一颗★怪物强化约 1.6 倍，奖励约 1.75 倍
                    monster.maxHp = Math.floor(monster.maxHp * 1.6);
                    monster.hp = monster.maxHp;
                    monster.damage = Math.floor(monster.damage * 1.6);
                    monster._baseDamage = monster.damage;
                    monster.expReward = Math.floor(monster.expReward * 1.75);
                    monster.goldReward = Math.floor(monster.goldReward * 1.75);
                }
...
function rollEquipmentDropAtMonster(monster, gameInstance, dropChance) {
    if (!gameInstance || !monster || typeof Equipment === 'undefined') return;
    const p = typeof dropChance === 'number' ? dropChance : 0.3;
    if (p <= 0 || Math.random() >= p) return;

    let newEq = null;
    if (typeof window.generateProceduralEquipment === 'function' && window.BASE_TYPES) {
        const player = gameInstance.player;
        const classId = player && player.classData && typeof window.getPlayerBaseClassId === 'function'
            ? window.getPlayerBaseClassId(player.classData) : null;
        const isBoss = !!(monster.isBoss || monster.type === 'boss' || (monster.name && /boss|守塔/i.test(monster.name)));
        const isElite = !!(monster.isElite || monster.elite);
        const monsterTier = isBoss ? 'boss' : isElite ? 'elite' : 'normal';
        newEq = window.generateProceduralEquipment({
            monsterLevel: monster.level != null ? monster.level : 1,
            playerClass: classId,
            classId: player && typeof window.getActiveClassId === 'function' ? window.getActiveClassId(player.classData) : classId,
            monsterTier
        });
    }
...
class Player {
    constructor(x, y, gameInstance = null) {
        ...
        this.classResource = null;
        this.skillCooldowns = {};
        this.skillEnhanceLevels = {};
        this.chronicleUnlocked = [];
        this.chronicleRelics = [];
        this.storedPowers = [];
        this.talentAllocations = {};
        this.displayName = '冒险者';
        this.tutorialFlags = {};
        this.skillHotbar = null;
        this.lastAttackTime = 0;
        ...
        // 装备（10 槽位）
        this.equipment = (typeof window.createEmptyEquipmentSlots === 'function')
    ...
    updateStats() {
        if (typeof window.normalizeClassData === 'function' && this.classData) {
            this.classData = window.normalizeClassData(this.classData);
        }
        ...
        // 重置基础属性（职业驱动成长）
        const classStats = (typeof window.computeClassBaseStats === 'function')
            ? window.computeClassBaseStats(this.classData, this.level)
            : null;
        if (classStats) {
            this.maxHp = classStats.hp;
            this.baseAttack = classStats.attack;
            this.baseDefense = classStats.defense;
            this.baseMagicAttack = classStats.magicAttack;
            this.baseMagicDefense = classStats.magicDefense;
            this.baseCritRate = classStats.critical;
            this.baseDodge = classStats.dodge;
        } else {
            this.maxHp = 100;
            this.baseAttack = 10;
            this.baseDefense = 0;
            this.baseMagicAttack = 5;
            this.baseMagicDefense = 5;
        ...
        let classMoveBonus = classStats && classStats.speed ? classStats.speed * 2 : 0;
        // 确保使用最新的 CONFIG.PLAYER_SPEED 值
        const baseSpeed = window.CONFIG ? window.CONFIG.PLAYER_SPEED : CONFIG.PLAYER_SPEED;
        ...
        let moveSpeedPercent = classMoveBonus; // 移动速度百分比加成
        ...
        Object.values(this.equipment).forEach(eq => {
            if (eq) {
                ...
                const scale = (v) => (typeof v === 'number' && affMult !== 1 ? Math.round(v * affMult) : v);
                if (eq.stats.health) this.maxHp += scale(eq.stats.health);
                if (eq.stats.attack) this.baseAttack += scale(eq.stats.attack);
                if (eq.stats.magicAttack) this.baseMagicAttack += scale(eq.stats.magicAttack);
                if (eq.stats.defense) this.baseDefense += scale(eq.stats.defense);
                if (eq.stats.magicDefense) this.baseMagicDefense += scale(eq.stats.magicDefense);
        ...
        this.buffs.forEach(buff => {
            ...
            if (buff.effects.damageReduction) this.damageReductionPercent += buff.effects.damageReduction;
            ...
            if (buff.effects.attackSpeedPercent) {
                this.baseAttackSpeed += Math.floor(this.baseAttackSpeed * buff.effects.attackSpeedPercent / 100);
            }
        ...
        if (typeof window.applyTalentStatsToPlayer === 'function') {
            const talentMove = window.applyTalentStatsToPlayer(this);
            if (talentMove) moveSpeedPercent += talentMove;
        }
        ...
        if (typeof window.applyBuildPassiveStats === 'function') {
            window.applyBuildPassiveStats(this);
        }
        ...
        this.effectiveAttack = (typeof window.getPlayerEffectiveAttack === 'function')
            ? window.getPlayerEffectiveAttack(this)
            : this.baseAttack;
    ...
    _evaluateCombatPowerFromSnapshot(s) {
        ...
        power += (s.baseMagicAttack || 0) * 2.5;
        const defenseValue = (s.baseDefense + (s.baseMagicDefense || 0) * 0.5) * 0.5;
    ...
    _computeStableCombatPowerValue() {
        const lv = Math.max(1, this.level | 0);
        const classStats = (typeof window.computeClassBaseStats === 'function')
            ? window.computeClassBaseStats(this.classData, lv)
            : null;
        let maxHp = classStats ? classStats.hp : 100 + (lv - 1) * 20;
        let baseAttack = classStats ? classStats.attack : 10 + (lv - 1) * 2;
        let baseMagicAttack = classStats ? classStats.magicAttack : 5;
        if (!classStats && lv > 20) {
            baseAttack += Math.floor((lv - 20) * 4.5);
        }
        let baseDefense = classStats ? classStats.defense : 0;
        let baseMagicDefense = classStats ? classStats.magicDefense : 5;
        let baseCritRate = classStats ? classStats.critical : 5;
        let baseCritDamage = 20;
        let baseDodge = classStats ? classStats.dodge : 0;
    ...
    move(dx, dy) {
        try {
            ...
            if (this.isDashing) {
                ...
                const slowImmune = this.buffs && this.buffs.some(b => b.expireTime > now && b.effects && b.effects.slowImmune);
                if (!slowImmune && this.slowEffects && this.slowEffects.length) {
        ...
        targetsHit.forEach(monster => {
            ...
            hit = true;
                    ...
                    monsters.forEach(m => {
                        ...
                        if (aoeDistance <= aoeRange) {
                            ...
                            if (critRoll < this.baseCritRate) {
                                aoeDmg = applyCritDamageMultiplier(aoeDmg, this.baseCritDamage);
                            }
                ...
                if (skill.guaranteedCrit) {
                    isCrit = true;
                    damage = applyCritDamageMultiplier(damage, this.baseCritDamage, 1 + (skill.critDamageBonus || 0));
                } else if (skill.critBonus) {
                    const critRoll = Math.random() * 100;
                    if (critRoll < critRate) {
                        isCrit = true;
                        damage = applyCritDamageMultiplier(damage, this.baseCritDamage, 1 + skill.critBonus + (skill.critDamageBonus || 0));
                    }
                } else {
                    const critRoll = Math.random() * 100;
                    if (critRoll < critRate) {
                ...
                if (this.gameInstance && mainSkillDmg > 0) {
                    this.gameInstance.triggerHitImpact(monster.x, monster.y, {
                        isRanged: isPlayerWeaponRanged(weapon, this),
                        isCrit: isCrit,
                        target: monster,
                        sourceX: this.x,
                        sourceY: this.y,
    ...
    attack(monsters) {
        try {
            ...
            this.lastAttackTime = now;
            if (this.gameInstance && typeof window.notifyTutorialEvent === 'function') {
                window.notifyTutorialEvent(this.gameInstance, 'attack_once');
            }
            if (typeof window.onPlayerBasicAttackResource === 'function') {
                window.onPlayerBasicAttackResource(this);
            }

            if (typeof window.tryPerformClassBasicAttack === 'function') {
                const entityBasic = window.tryPerformClassBasicAttack(this, monsters, this.gameInstance);
                if (entityBasic) {
                    if (this.gameInstance && this.gameInstance.soundManager) {
            ...
            if (isPlayerWeaponRanged(weapon, this)) {
                const PI = Math.PI;
    ...
    takeDamage(amount, attacker = null, isCrit = false) {
        try {
            ...
            if (this.buffs && this.buffs.length > 0 && actualDamage > 0) {
                for (const buff of this.buffs) {
                    if (buff.expireTime <= nowShield || !buff.shieldRemaining || buff.shieldRemaining <= 0) continue;
            ...
            if (typeof window.onBuildPlayerHit === 'function') {
                window.onBuildPlayerHit(this, attacker);
            }
            ...
            if (typeof window.onPlayerHitTakenResource === 'function') {
                window.onPlayerHitTakenResource(this);
            }
    ...
    applySetAttackEffects(damage, monster) {
        ...
        for (const [setId, setEffect] of Object.entries(this.setSpecialEffects)) {
            ...
            let effect = setData.effects[String(setEffect.pieceCount)] || setData.effects[setEffect.pieceCount];
            if (!effect || !effect.special) continue;
            if (effect.special === 'fire_nova') effect = Object.assign({}, effect, { special: 'flameExplosion' });
            if (effect.special === 'chain_strike') effect = Object.assign({}, effect, { special: 'chainLightning' });
            if (effect.special === 'frost_touch') effect = Object.assign({}, effect, { special: 'freezeChance', freezeProcChance: 0.25, freezeDurationMs: 2000 });
            if (effect.special === 'star_stack') effect = Object.assign({}, effect, { special: 'killBuff', killBuffStatFrac: 0.05, killBuffDurationMs: 10000 });
    ...
    tickDeepSetPeriodicEffects(gameInstance) {
        if (!gameInstance || !this.setSpecialEffects || typeof resolveSetDefinition !== 'function') return;
        const now = Date.now();
    ...
    processAttackTraits(monster, damage, isCrit) {
        ...
        const isStoolDummy = monster instanceof TrainingDummy;
        let damageType = 'physical'; // 默认物理伤害
        const meleeW = !isPlayerWeaponRanged(this.equipment.weapon, this);
        const rangedW = isPlayerWeaponRanged(this.equipment.weapon, this);
    ...
    draw(ctx) {
        ...
        if (!isPlayerWeaponRanged(this.equipment.weapon, this) && this.slashStartTime > 0) {
