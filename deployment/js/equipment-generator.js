...
(function () {
    ...
    window.rollEquipmentAffixes = function rollEquipmentAffixes(slot, quality, level, classId) {
        return rollAffixes(slot, quality, level, classId);
    };

    window.rebuildProceduralEquipmentStats = function rebuildProceduralEquipmentStats(equipment, classId) {
        if (!equipment || !equipment.baseTypeId) return false;
        const bt = window.BASE_TYPES;
        if (!bt) return false;
        const def = (bt.weapons && bt.weapons[equipment.baseTypeId])
            || (bt.offHand && bt.offHand[equipment.baseTypeId])
            || (bt.armor && bt.armor[equipment.baseTypeId])
            || (bt.accessories && bt.accessories[equipment.baseTypeId]);
        if (!def) return false;
        const eqLevel = Math.max(1, equipment.level || 1);
        let stats = {};
        if (def.baseStatsPerLevel) {
            stats = scaleStatsPerLevel(def.baseStatsPerLevel, eqLevel);
        } else if (def.baseStats) {
        ...
        mergeStats(stats, def.implicit);
        mergeStats(stats, affixesToStats(equipment.prefixes, equipment.suffixes));
        equipment.stats = stats;
        equipment.baseStats = JSON.parse(JSON.stringify(stats));
        const affinity = equipment.classAffinity || def.classAffinity || classId || null;
        equipment.name = composeEquipmentName(
            def.name,
            equipment.quality,
            equipment.prefixes,
            equipment.suffixes,
