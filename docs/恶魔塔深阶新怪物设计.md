...
本文档汇总恶魔塔（240 层、怪物等级上限 60）的**深阶新怪策划**与**工程侧已落地**内容的对照。表中「建议/暂定」条目未必已全部实装；**以 `config/monster-config.json` 与 [`怪物一览.md`](./怪物一览.md) 为准**。`deployment/` 目录需与主工程配置保持同步。
...
- **状态**：本文兼顾策划表与**已落地**对照；新增/改动怪物请以 **`monster-config.json`** 与 **`怪物一览.md`** 为权威。
