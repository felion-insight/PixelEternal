...
class Equipment {
    ...
    generateEquipmentTraits() {
        ...
        if (nameTraits[this.name]) {
            ...
            const slotType = this.slot === 'weapon' ? 'weapon' : 
                           ['helmet', 'body', 'legs', 'feet'].includes(this.slot) ? 'armor' : 'accessory';
    ...
    getTooltipHTML(currentEquipment = null) {
        ...
        // 显示套装信息（旧版名称匹配 + 新版 setId）
        const setId = getSetForEquipment(this.name);
        const setIdsToShow = setId ? [setId] : [];
...
function generateEquipments() {
    ...
    equipmentDefinitions.forEach(def => {
        ...
        } else if (['helmet', 'body', 'legs', 'feet'].includes(def.slot)) {
            // 防具：只保留生命值、防御力
