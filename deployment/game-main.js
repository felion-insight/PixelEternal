...
class Game {
            ...
            const handleStartClick = async () => {
                ...
                setTimeout(() => {
                    ...
                    requestAnimationFrame(() => {
                        ...
                        if (this.classUI) this.classUI.maybeShowClassSelectOnStart();
                    });
    ...
    init() {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (['1', '2', '3', '4'].includes(e.key) && !e.repeat && !this.player.isDashing) {
                this.useClassSkillHotbar(parseInt(e.key, 10) - 1);
            }
    ...
    handleInput() {
        try {
            ...
            // Shift 冲刺
            const dashPressed = this.keys['shift'];
    ...
    update() {
        try {
            ...
            if (typeof window.tickPlayerClassResource === 'function') {
                const inCombat = this.currentScene === SCENE_TYPES.TOWER || this.currentScene === SCENE_TYPES.TRAINING;
    ...
    closeBlacksmith() {
        const modal = document.getElementById('blacksmith-modal');
        modal.classList.remove('show');
        // 切换回普通模式        const codexModal = document.getElementById('codex-modal');
        const shopModal = document.getElementById('shop-modal');
        const inventoryModal = document.getElementById('inventory-modal');
        const guideModal = document.getElementById('guide-modal');
        if (!this.devMode && !codexModal.classList.contains('show') && 
     ...
     */
    useClassSkillHotbar(slotIndex) {
        if (this.paused) return;
        if (!window.hasPlayerClass(this.player.classData)) return;
        if (!this._canUseWeaponSkillForBattle()) return;
        const hotbar = window.getPlayerHotbarSkills(this.player);
        const skillDef = hotbar[slotIndex];
        if (!skillDef) return;
        const monsters = this._getSkillMonsters();
        if (!monsters) return;
        window.executeClassSkill(this.player, monsters, skillDef, this);
    }
    ...
    devCreateEquipmentFromDefinition(def, id) {
        ...
        } else if (['helmet', 'body', 'hands', 'legs', 'feet'].includes(def.slot)) {
            stats.health = def.health || 0;
            stats.defense = def.defense || 0;
        } else if (['amulet', 'ring', 'belt'].includes(def.slot)) {
