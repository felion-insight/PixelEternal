...
(function () {
    ...
    window.normalizeClassData = function normalizeClassData(raw) {
        ...
        let baseClass = raw.baseClass;
        let firstAdvancement = raw.firstAdvancement || null;
        let secondAdvancement = raw.secondAdvancement || null;
        ...
        if (!validBases.includes(baseClass)) {
            console.warn('normalizeClassData: 无法识别的职业数据，已重置', raw);
            return {
                baseClass: null,
                firstAdvancement: null,
                secondAdvancement: null
        ...
        if (firstAdvancement && cfg && cfg.firstAdvancements && cfg.firstAdvancements[firstAdvancement]) {
            const firstDef = cfg.firstAdvancements[firstAdvancement];
            if (firstDef.baseClass !== baseClass) firstAdvancement = null;
        } else if (firstAdvancement) {
            firstAdvancement = null;
        }

        if (secondAdvancement && cfg && cfg.secondAdvancements && cfg.secondAdvancements[secondAdvancement]) {
            const secDef = cfg.secondAdvancements[secondAdvancement];
            if (!firstAdvancement || secDef.firstAdvancement !== firstAdvancement) {
                secondAdvancement = null;
            }
        } else if (secondAdvancement) {
            secondAdvancement = null;
        }
